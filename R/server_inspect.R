server_inspect <- function(input, output, session, state, plates) {
  ...
  observeEvent(state$screen, {
    ...
    req(length(plates()) > 0)
    updateSelectInput(session, "active_plate", choices = names(plates()))
  })

  # --- Plotting ---
  output$plate_preview <- renderPlot({
    req(input$active_plate)
    plate <- plates()[[input$active_plate]] %>% filter(!is.na(value))
    ...
    # Factor rows
    expanded_plate <- expanded_plate %>% mutate(row = factor(row, levels = rev(sort(unique(row)))))
    ...
    ggplot(expanded_plate) +
      ...
      geom_rect_pattern(data = filter(expanded_plate, is_blank),
      ...
      geom_rect_pattern(data = filter(expanded_plate, is_control),
                        aes(xmin = xmin, xmax = xmax, ymin = ymin, ymax = ymax, fill = group),
                        pattern_color = "grey", pattern_angle = 45, pattern_density = 0.1,
                        pattern_spacing = 0.02, pattern_alpha = 0.8, show.legend = c(fill=TRUE, pattern=FALSE)) +
      geom_tile(data = plate, aes(x = col, y = row), fill = NA, color = "white", linewidth = 1.2) +
      geom_text(aes(x = col, y = row, label = value), size = 3) +
      scale_fill_manual(values = palette, breaks = non_std_labels, labels = identity, na.value = "grey") +
      theme_void() +
      theme(
        ...
        plot.caption = element_text(hjust = 0.5, size = 11, color = "grey", margin = margin(t = 8))
      ) +
      labs(fill = "Label",
  ...
  observeEvent(input$mark_control, { if (isTRUE(input$mark_control)) { updateCheckboxInput(session, "mark_blank", value = FALSE); updateCheckboxInput(session, "mark_standard", value = FALSE) } })
  observeEvent(input$mark_blank, { if (isTRUE(input$mark_blank)) { updateCheckboxInput(session, "mark_control", value = FALSE); updateCheckboxInput(session, "mark_standard", value = FALSE) } })
  observeEvent(input$mark_standard, { if (isTRUE(input$mark_standard)) { updateCheckboxInput(session, "mark_blank", value = FALSE); updateCheckboxInput(session, "mark_control", value = FALSE) } })

  # Disable next button until label applied
  observe({
    if (state$screen != "inspect") { shinyjs::disable("to_normalize"); return() }
    req(input$active_plate)
    plate <- plates()[[input$active_plate]]
    has_group <- any(lengths(plate$labels) > 0 | lengths(plate$control_groups) > 0 | lengths(plate$blanks) > 0)
    shinyjs::toggleState("to_normalize", condition = has_group)
  })
  ...
  observeEvent(input$apply_label, {
    ...
    if (isTRUE(input$mark_standard)) {
      ...
      if (is.na(conc)) { showNotification("Standard labels must be numeric concentrations.", type = "error"); return() }
      plate$is_standard[idx] <- TRUE; plate$is_control[idx] <- FALSE; plate$is_blank[idx] <- FALSE; plate$is_label[idx] <- FALSE
      plate$standards[idx] <- conc; plate$standard_units[idx] <- input$standard_units
      plate$labels[idx] <- list(character(0)); plate$control_groups[idx] <- list(character(0)); plate$blanks[idx] <- list(character(0))
    } else if (isTRUE(input$mark_blank)) {
      plate$is_blank[idx] <- TRUE; plate$is_control[idx] <- FALSE; plate$is_label[idx] <- FALSE
      plate$control_groups[idx] <- list(character(0)); plate$labels[idx] <- list(character(0))
      plate$blanks[idx] <- lapply(plate$blanks[idx], function(x) unique(c(x, input$new_label)))
    } else if (isTRUE(input$mark_control)) {
      plate$is_control[idx] <- TRUE; plate$is_blank[idx] <- FALSE; plate$is_label[idx] <- FALSE
      plate$blanks[idx] <- list(character(0)); plate$labels[idx] <- list(character(0))
      plate$control_groups[idx] <- lapply(plate$control_groups[idx], function(x) unique(c(x, input$new_label)))
    } else {
      plate$is_control[idx] <- FALSE; plate$is_blank[idx] <- FALSE; plate$is_label[idx] <- TRUE
      plate$control_groups[idx] <- list(character(0)); plate$blanks[idx] <- list(character(0))
  ...
  # Clear Labels
  observe({
    if (state$screen != "inspect") { shinyjs::disable("clear_label"); return() }
  ...
  observeEvent(input$clear_label, {
    ...
    idx <- paste(plate$row, plate$col) %in% paste(brushed$row, brushed$col)
    plate$labels[idx] <- list(character(0)); plate$control_groups[idx] <- list(character(0)); plate$blanks[idx] <- list(character(0))
    plate$is_control[idx] <- FALSE; plate$is_blank[idx] <- FALSE; plate$is_label[idx] <- FALSE
    plate_list[[input$active_plate]] <- plate
    plates(plate_list)
  })
